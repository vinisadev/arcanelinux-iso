...
old_buildiso=$(grep -oP "arcaneVersion='v\d{2}\.\d{2}\.\d{2}'" build-scripts/build-the-iso.sh | grep -oP 'v\d{2}\.\d{2}\.\d{2}' | head -1)
old_profiledef=$(grep -oP 'arcane-v\d{2}\.\d{2}\.\d{2}' archiso/profiledef.sh | grep -oP 'v\d{2}\.\d{2}\.\d{2}' | head -1)
...
# Replace entire arcaneVersion='...' line
sed -i "s|\(.*arcaneVersion='\)[^']*\('.*\)|\1$newversion\2|" build-scripts/build-the-iso.sh

# Replace entire iso_label="arcane-..." line
sed -i "s|^iso_label=\"arcane-.*\"|iso_label=\"arcane-$newversion\"|" archiso/profiledef.sh
